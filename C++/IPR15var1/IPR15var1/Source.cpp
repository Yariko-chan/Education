...
int main()
{
	...
	// active iterator usage
	cout << "\nACTIVE ITERATOR\n";
	cout << "The stack is: ";
	Iterator<int> begin = s.begin();
	Iterator<int> end = s.end();
	while (begin !=  end) {
		cout << *begin << " ";
		++begin;
	}
	...
	Iterator<int> min = s.min(s.begin(), s.end());
	Iterator<int> max = s.max(s.begin(), s.end());
	cout << "Stack min = " << *min << "\n";
	cout << "Stack max = " << *max << "\n";
	cout << "Stack avg = " << s.avg(s.begin(), s.end()) << "\n";
	cout << "Replace max with min\n";
	s.replace(max, *min);

	// passive iterator usage
	cout << "\nPASSIVE ITERATOR\n";
	cout << "The stack is: ";
	PassiveIterator<int> p_iterator(&s);
	p_iterator.apply(&print);
